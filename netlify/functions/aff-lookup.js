...
//
// SECURITY: This endpoint now REQUIRES a signed token tied to the email being
// looked up. The token is generated by aff-request-link.js (sent via email).
// Without a valid, non-expired token, the request is rejected — preventing
// anyone from viewing an affiliate's stats by simply typing their email.

const crypto = require('crypto');

// Verify a token issued by aff-request-link.js.
// Returns { valid, email, reason }.
function verifyToken(token, providedEmail, secret) {
  ...
  let decoded;
  try { decoded = Buffer.from(payloadB64, 'base64url').toString('utf8'); }
  catch { return { valid: false, reason: 'malformed' }; }

  const [emailFromToken, expStr] = decoded.split('|');
  const exp = parseInt(expStr, 10);
  if (!emailFromToken || !exp) return { valid: false, reason: 'malformed' };

  // Re-compute expected signature and compare in constant time.
  const expected = crypto.createHmac('sha256', secret).update(payloadB64).digest('hex').slice(0, 32);
  let sigOk = false;
  try {
    sigOk = sig.length === expected.length &&
      crypto.timingSafeEqual(Buffer.from(sig, 'hex'), Buffer.from(expected, 'hex'));
  } catch { sigOk = false; }
  if (!sigOk) return { valid: false, reason: 'bad-signature' };

  if (Date.now() > exp) return { valid: false, reason: 'expired' };
  if (providedEmail && providedEmail.toLowerCase() !== emailFromToken.toLowerCase()) {
    return { valid: false, reason: 'email-mismatch' };
  }

  return { valid: true, email: emailFromToken };
}
...
exports.handler = async function(event) {
  ...
  // ── Token check ─────────────────────────────────────────────────────────
  // Without AFFILIATE_AUTH_SECRET set (e.g. older deploy), fall open — but
  // log loudly. New deploys MUST set the secret in Netlify env.
  if (!SECRET || SECRET.length < 32) {
    console.warn('[aff-lookup] AFFILIATE_AUTH_SECRET missing — falling back to email-only lookup (INSECURE). Set the env var.');
  } else {
    const result = verifyToken(token, email, SECRET);
    if (!result.valid) {
      console.log(`[aff-lookup] Token rejected: ${result.reason} for ${email}`);
      return {
        statusCode: 401,
        headers,
        body: JSON.stringify({ error: 'unauthorized', reason: result.reason })
      };
    }
  }
